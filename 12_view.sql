...
-- view 라는 사강의 테이블은 베이스 테이블(tbl_menu) 의 정보가 변경되면
-- view 의 결과도 같이 변경된다.
...
-- view로 dml 명렁어로 조작이 불가능한 경우
-- 1. 뷰 정의에 포함되지않은 컬럼을 조작하는 경우
-- 2. 뷰에 퐇핳ㅁ되지 않은 컬럼 붕 베이스 테이블 컬럼이 not null 제약조건이 지정된 경우
-- 3. 산술표현식이 정의된 경우
-- 4. join 을 이용해 여러 테이블을 연결한 경우
-- 5. distinct를 포함한 경우
-- 6. 그룹함수(avg, sum등등) 나 group by정릉 포함한 경우
