...
SELECT format(123123123123.12312312331,3);

-- insert(기준 문자열, 위치, 길이, 삽입할 문자열)
SELECT INSERT('내 이름은 아무개 입니다.', 7,3,'이지은');
...
-- 방향 : leading(앞), both(양쪽), trailin(뒤) 해당 방향에 대한 지정한 문자열을 제거할 수 있다.
...
SELECT year(now()), month(now()),day(now()), microsecond(now());

-- datediff(날짜 1, 날짜 2)
-- timediff(날짜 1 or 시간 1 , 날짜 2 또는 시간 2)
-- datediff: 날짜 1 - 날짜 2의 일수를 반환
-- timediff : 시간1 - 시간 2의 결과
